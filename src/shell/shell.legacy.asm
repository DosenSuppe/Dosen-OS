...
; Replace what the user has currently typed with the supplied string.
; The "> " prompt is left untouched -- only the input area is rewritten.
; Use this for history navigation, tab completion, etc.
;
; Source string layout (matches String.PrintString / CopyStringBuffer):
;   word[0]       = length
;   word[1..len]  = characters
;
; @REA pointer to the source string
ReplaceInput:
    PUSH REA
    PUSH REB
    PUSH REC
    ...
    ; --- erase whatever the user has on screen ---
    ; One Backspace per buffered char walks the cursor back to the
    ; start-of-input boundary. We'll overwrite the buffer length below.
    LDI REC, [REB]
    LDI REN, #0
    EraseLoop:
        CMP REC, REN
    ...
    LDI REC, [REA]              ; REC = new length
    STR REB, REC                ; buffer[0] = new length

    LDI REN, #0
    CMP REC, REN
    JP_EQ WriteDone             ; empty replacement -> nothing more to do

    LDI REN, #1
    WriteLoop:
        PUSH REA
        ADD REA, REN
        LDI REA, [REA]          ; REA = source[REN]

        PUSH REB
        ADD REB, REN
        STR REB, REA            ; buffer[REN] = char
        POP REB
...
Try_del:
    ; "del" -- delete file(/ directory once directories are supported)
    MOV REC, REO
    LDI REB, [REC]
...
Try_run:
    ...
    ADD REC, REP
    LDI REB, [REC]
    LDI REA, Characters.n
    CMP REA, REB
    JP_NEQ Execute_Done
    CALL commands.CMD_RUN
    JP Execute_Done
